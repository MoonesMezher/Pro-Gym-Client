...
const Header = () => {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50 bg-main">
      <nav
        aria-label="Global"
        className="flex items-center justify-between py-5 container mx-auto px-4"
      >
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="text-sm/6 font-semibold text-white hover:text-skin"
            >
              {item.name}
            </Link>
          ))}
      ...
      >
        ...
        <DialogPanel className=" bg-[#ddd] fixed inset-y-0 right-0 z-50 w-full overflow-y-auto  p-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
